...
export async function listUnspent(
    ...
    if (poolID === 1000) {
        //Não usar a nova API por enquanto.
        let UTXOconf = await listUnspentConfWOC(addOrScriptHash, homenetwork)
        let UTXOunconf = await listUnspentUnconfWOC(
            addOrScriptHash,
            homenetwork
        )
    ...
    while (!bcFinish && cycle < npools * 2) {
        ...
        try {
            ...
            if (
                ...
                if (resp.indexOf('[]') !== -1) {
                    let res = JSON.parse(
                        '[{"height":-2,"tx_pos":-1,"tx_hash":"","value":0}]'
                    )
                    ...
                    res = JSON.parse(
                        resp.substring(resp.indexOf('['), resp.indexOf(']') + 1)
                    )
                    utxos = res.map((item: any) => ({
                        //utxos = res.body.map((item: any) => ({
                        height: -1,
...
//export async function listUnspent(addOrScriptHash: any, homenetwork: any): Promise <string>
export async function listUnspentUnconfWOC(
    addOrScriptHash: any,
    homenetwork: any
): Promise<UTXO[]> {
    //console.log(`Message received in worker: ${event.data}`);
...
//export async function listUnspent(addOrScriptHash: any, homenetwork: any): Promise <string>
export async function listUnspentConfWOC(
    addOrScriptHash: any,
    homenetwork: any
): Promise<UTXO[]> {
    //console.log(`Message received in worker: ${event.data}`);
...
export async function getSpentOutput(
    txid: any,
    vout: number,
    homenetwork: any
): Promise<STXO[]> {
    ...
    if (homenetwork === bsv.Networks.testnet) {
        urlAdress01 =
            'https://api.whatsonchain.com/v1/bsv/test/tx/' +
            txid +
            '/' +
            position +
            '/spent'
...
//export async function listUnspent(addOrScriptHash: any, homenetwork: any): Promise <string>
export async function scriptHistory(
    addOrScriptHash: any,
    homenetwork: any
): Promise<UTXO[]> {
    //console.log(`Message received in worker: ${event.data}`);
    ...
    while (!bcFinish && cycle < npools * 2) {
        switch (poolID) {
            case 0:
                url = new URL(
                    urlAdress01 + addOrScriptHash + '/confirmed/history'
                )
                //TXJson = `{"txhex": "${TxHexBsv}" }`;
                break
            default:
          ...
          */
                ...
                if (resp.indexOf('Not Found') !== -1) {
                    ...
                    console.log(
                        'res: ',
                        resp.substring(
                            resp.indexOf('"result":[') + 9,
                            resp.length -
                                (resp.length - resp.indexOf(',"error"'))
                        )
                    )
...
//export async function listUnspent(addOrScriptHash: any, homenetwork: any): Promise <string>
export async function scriptHistoryUnc(
    addOrScriptHash: any,
    homenetwork: any
): Promise<UTXO[]> {
    //console.log(`Message received in worker: ${event.data}`);
    ...
    while (!bcFinish && cycle < npools * 2) {
        switch (poolID) {
            case 0:
                url = new URL(
                    urlAdress01 + addOrScriptHash + '/unconfirmed/history'
                )
                //TXJson = `{"txhex": "${TxHexBsv}" }`;
                break
            default:
